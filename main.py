...
def dayoff(login_days, dayoff_data, user, invalid_dayoff_list):
    dayoff_users = dayoff_data['USER'].unique()
    for dayoff_user in dayoff_users:
        if user == dayoff_user:
            ...
            for dayoff in dayoff_dates:
                ...
                end_date = dayoff[1] if isinstance(dayoff[1], datetime.datetime) else pd.to_datetime(dayoff[1],
                                                                                                     format='%d/%m/%Y')
                for login_date in login_days:
                    if start_date <= pd.to_datetime(login_date) <= end_date:
                        invalid_user = {'user': user, 'start_date': start_date, 'end_date': end_date,
                                        'login_date': login_date}
                        if invalid_user.get('start_date') not in invalid_dayoff_list:
                            invalid_dayoff_list.append(invalid_user)


def checkout(login_days, checkout_data, user, invalid_checkout_list):
    checkout_users = checkout_data['USER'].unique()
...
if __name__ == '__main__':
    login_data_1 = pd.read_excel('part_1.xlsx', sheet_name='login_data')
    login_data_2 = pd.read_excel('part_2.xlsx', sheet_name='login_data')
    ...
    # checkout
    checkout_data = pd.read_excel('part_1.xlsx', sheet_name='checkout_data')
    # dayoff
    dayoff_data = pd.read_excel('part_1.xlsx', sheet_name='dayoff_data')
    ...
    with pd.ExcelWriter("checking_output.xlsx") as writer:
